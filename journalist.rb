...
@nombre_aude = 0
journalistes.each { |journalist|
	if journalist.include?("aude") || journalist.include?("aude".capitalize) || journalist.include?("aude".upcase) 
		@nombre_aude += 1
...
@nombre_maj = 0
journalistes.each { |journalist|
	tableau_lettres_journaliste = journalist.split('') 
		if tableau_lettres_journaliste[1].match(/[A-Z]/)
			@nombre_maj += 1
		end
}
...
@au_moins_une_maj = 0
journalistes.each { |journalist|
	if journalist.count("A-Z") > 0
		@au_moins_une_maj += 1
	end

}
puts @au_moins_une_maj
# => 215
...
journalistes.each { |journalist|
	tableau_lettres_journaliste = journalist.split('')
	tableau_lettres_journaliste.each { |lettre|
		if lettre == "_"
			@nombre_de_ += 1
		end
...
journalistes_minuscules = []

journalistes.each { |journalist|
	journalistes_minuscules << journalist.downcase 

}

puts journalistes_minuscules.sort 
